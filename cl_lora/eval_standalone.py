...
def run_eval_from_manifest(
    ...
    *,
    ...
    if raw_adapter_paths is not None:
        ...
        # SAPT eval path: load every stage adapter as a parallel named adapter and
        # wrap with SAPTWrapper so generation routes through the attention router.
        # Selected when the manifest declares cl_method=sapt and a router
        # checkpoint is present; otherwise fall back to the sequential-merge
        # reconstruction (which also bakes SD-LoRA state when present).
