...
def run_sequence(
    ...
    if resume and partial_path.exists():
        ...
        if completed > 0:
            ...
            _resume_cls = CL_METHOD_REGISTRY.get(str(cl_method_name).lower())
            if bool(getattr(_resume_cls, "keeps_parallel_adapters", False)):
                # SAPT keeps adapters parallel -- load every prior adapter as a
                # named adapter without merging (base stays pristine, no absorption
                # replay). The router, if already trained, is restored later via
                # cl_method.load(cl_state_dir).
                from peft import PeftModel

                base = load_base_model(model_name=str(base_model_ckpt), hf_token=HF_TOKEN)
                peft_model = None
                for i, ap in enumerate(adapter_paths_resume):
                    name = _resume_cls.adapter_name_for_stage(i + 1)
                ...
                model = peft_model
            elif bool(getattr(_resume_cls, "requires_no_merge", False)):
                # SD-LoRA never merges: the accumulated state lives in the forward
                # (W0 + sum_i s_i D_i) and is restored from cl_state by
                # cl_method.load() + before_init() below. Baking the prior blocks
                # into the base here -- which load_model_with_adapters does when it
                # finds sd_lora_state.pt next to the last adapter -- would count
                # every previous task twice for the resumed stage.
                model = load_base_model(model_name=str(base_model_ckpt), hf_token=HF_TOKEN)
            else:
    ...
    # SAPT (parallel experts + shared-attention router) needs orchestrator-level
    # steps the other methods don't: parallel-adapter resume (above), a router
    # training pass (run_arm) each stage, and a SAPTWrapper at eval time.
    sapt_active = bool(getattr(cl_method, "keeps_parallel_adapters", False))
    print(f"CL method: {cl_method.name} | state dir: {cl_state_dir} | parallel_adapters={sapt_active}")
    ...
    for idx in range(start_stage, len(sequence.tasks) + 1):
        ...
        # SAPT post-stage: grow + train the shared-attention router on
        # pseudo-samples generated by every loaded expert. Must run BEFORE
        # cl_method.save() so the router is persisted into cl_state/sapt/.
        ...
        if sapt_active:
            # Prefer the per-stage router snapshot (enables full-matrix AP eval);
            # fall back to the canonical router.pt.
        ...
        if train_only:
            ...
            if sapt_active:
                # Wrap the un-merged parallel-adapter PeftModel so forward/generate
                # route through the shared-attention router instead of summing every
                # expert unweighted.
                try:
                ...
                router_packed = cl_method.get_router_packed()
                if router_packed is None:
                    print("  WARNING: SAPT eval requested but no router available; using bare model.")
                    eval_model = model
                else:
                    router = SAPTRouter.from_packed(router_packed)
                    eval_model = SAPTWrapper(
                        model,
                        router.to(next(model.parameters()).device),
                        cl_method.all_adapter_names(),
                    )
            else:
                eval_model = model
...
def main() -> None:
    ...
    parser.add_argument("--cl-method",
        ...
        help="Continual-learning training method (composes with any LoRA init). "
             ...
             "(pair with --slice-init for SD-LoRA + SLICE). 'sapt' keeps every "
             "task's LoRA as a live parallel expert (no merge) and mixes them with "
             "a shared-attention router trained by ARM.")
    ...
    parser.add_argument("--cl-sapt-key-dim", type=int, default=64,
        help="SAPT router key/query dimensionality. Used only when --cl-method=sapt.")
    parser.add_argument("--cl-sapt-arm-n-samples", type=int, default=64,
        help="Pseudo-samples per task generated during SAPT ARM router training.")
    parser.add_argument("--cl-sapt-arm-max-new-tokens", type=int, default=32,
        help="Max new tokens per pseudo-sample during SAPT ARM generation.")
    parser.add_argument("--cl-sapt-arm-max-input-length", type=int, default=128,
        help="Max input length when tokenizing seed prompts for SAPT ARM generation.")
    parser.add_argument("--cl-sapt-arm-n-epochs", type=int, default=3,
        help="Router training epochs over the SAPT ARM pseudo-sample pool.")
    parser.add_argument("--cl-sapt-arm-batch-size", type=int, default=4,
        help="Batch size for SAPT router optimization steps.")
    parser.add_argument("--cl-sapt-arm-learning-rate", type=float, default=1e-3,
        help="SAPT router AdamW learning rate.")
    parser.add_argument("--cl-sapt-seed-prompts-per-task", type=int, default=32,
        help="How many training prompts to cache per task for SAPT ARM seeding.")
    ...
    payload = run_sequence(
        ...
        cl_method_kwargs={
            ...
            "max_seq_length": 256,
            "seed": args.seed,
            # SAPT-specific (filtered out by build_cl_method for other methods):
