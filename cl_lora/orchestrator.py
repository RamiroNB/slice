...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Per-device training batch size passed to the HF Trainer.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable gradient checkpointing in the HF Trainer (saves activation memory at ~30%% speed cost).",
    )
    parser.add_argument(
        "--torch-compile",
        action="store_true",
        help="Enable torch.compile in the HF Trainer (~10-30%% speedup, adds ~2 min warmup per stage).",
    )
    parser.add_argument(
        "--slice-micro-batch-size",
        type=int,
        default=None,
        help="Micro-batch size for gradient accumulation during slice init. "
             "Splits each batch into smaller chunks to reduce peak activation memory. "
             "Mathematically equivalent to the full batch. Default: no splitting.",
    )
    parser.add_argument(
        "--slice-batch-size",
        type=int,
        default=None,
        help="Per-step batch size for the slice-init gradient accumulation. "
             "Decoupled from --per-device-train-batch-size so the slice gradient can "
             "match a reference recipe independently of the training batch. "
             "Default: same as --per-device-train-batch-size.",
    )
